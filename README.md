...
运行时。插件只保留内置二维码登录，支持查询、点赞、评论、回复、发布、删除和
最近访客等能力。
...
插件不再包含 OneBot Cookie、OneBot HTTP、手动 Cookie 或定时发布/评论功能。
...
扫码 Cookie 失效后，执行 `qzone.logout`，再重新执行 `qzone.login`。
