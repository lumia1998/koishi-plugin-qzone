...
- ChatLuna 查询动态时下载图片并作为视觉内容传给支持图片输入的模型
- Koishi `看说说` 命令会将图片和视频下载到本地缓存后，以二进制图片/视频元素发送
...
- 媒体缓存：`data/qzone/temp/{image,model,video}/`，默认保留 7 天并每天清理
- OneBot Cookie 刷新 TTL：10 分钟
- 二维码登录超时：120 秒
- 二维码轮询间隔：2 秒
- 请求超时：10 秒
- 默认动态数量：5 条
- 每次最多向 ChatLuna 传递 9 张图片，总大小最多 16 MiB，单张最多 4 MiB
- 单次最多发布 9 张图片，每张最多 8 MiB
- `看说说` 每条动态最多发送 9 张图片（单张 8 MiB）和 3 个视频（单个 32 MiB）
- 允许下载的媒体域名：`*.qpic.cn`、`*.qlogo.cn`、`*.qq.com`、`*.gtimg.cn`
...
`看说说` 的原始媒体 URL 会保留在文本中便于追溯；插件同时将白名单内的图片和视频
下载至本地缓存，并通过 `h.image(buffer, mime)`、`h.video(buffer, mime)` 发送。因此不依赖
QQ 媒体直链是否能被目标适配器转发。最终是否显示为原生视频仍取决于所使用的平台和
OneBot 实现对视频元素的支持。
...
原文返回给模型。`qzone_feed` 和 `qzone_post` 会把缓存的图片作为多模态内容附加到工具
结果；如果当前 ChatLuna 模型不支持图片输入，图片会被模型适配器丢弃，但仍保留图片 URL。
...
- 扫码凭据失效后需要重新登录。
- 图片视觉理解依赖当前 ChatLuna 模型支持图片输入；仅支持文本的模型无法分析图片内容。
